...
class AdjustAttributeNumberIntent(intent.IntentHandler):
    intent_type = "AdjustAttributeNumber"
    ...
    async def async_handle(self, intent_obj: intent.Intent) -> intent.IntentResponse:
        ...
        name_list: list[str | None]  = slots.get("names", {}).get("value", [])
        area_name_list: list[str | None] = slots.get("areas", {}).get("value", [])
        floor_name: str | None = slots.get("floor", {}).get("value")
        
        _LOGGER.info(
            f"AdjustAttributeNumberIntent params: "
            f"attribute={attribute} delta={delta_raw} domain={domain} "
            f"name_list={name_list} area_name_list={area_name_list} floor_name={floor_name}"
        )
        ...
        for area_name in area_name_list:
            for name in name_list:
                ...
                for state in match_result.states:
                    _LOGGER.info(f"AdjustAttributeNumberIntent state: {state.as_dict_json}")
                    ...
                    try:
                        ...
                        # Perform adjustment.
                        _LOGGER.info(f"AdjustAttributeNumberIntent call target: {asdict(target)}")
